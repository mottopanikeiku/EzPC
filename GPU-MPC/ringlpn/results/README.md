...
| `secure_convert/` | Two-process `Z_M -> Z_2^bw` conversion: exact OT-backed edaBits/daBits, OT-backed Boolean triples, per-party output records, offline checks, and measured setup/correlation/online bytes and direction switches; plain loopback TCP, IKNP (not silent OT), ripple depth, and not yet production-integrated | `run_secure_convert_test.sh` |
| `dpf/` | Distributed DPF keygen artifacts: ideal-functionality protocol logic; two-process SCI/IKNP+Gilboa transport with measured bytes/direction switches; full-width four-call GPU AES parity and GPU-evaluated two-party keys; offline correctness/corruption/invalid-input controls. Direction switches are not network rounds; security reductions remain open. | `run_distributed_dpf_keygen.sh`, `run_two_party_dpf_keygen.sh`, `run_two_party_gpu_dpf.sh` |
...
| File | What it is |
|---|---|
| `reports/two_party_dpf_transport_memo_2026_07_29.md` | **Two-PROCESS keygen on a real transport**: SCI IKNP over TCP, Gilboa `Z_p` OLE, OT-based bit triples, OpenSSL-private-DRBG roots, 369/369 host-reference pairs, 88 GPU-evaluated full-width-AES pairs, versioned keys, measured bytes/direction switches, and explicit reduction/silent-OT/network-round boundaries |
| `reports/dealerless_ole_two_party_keys_memo_2026_07_29.md` | **M2 CORE GATE**: the real Figure 2 OLE engine validating on SPFSS keys generated by two OS processes over real OT, in all four deployed configurations; measured keygen direction switches/bytes/time per limb, the regular-vs-uniform keygen gap, and the oracles that remain |
...
| `reports/dealerless_orca_ringlpn_proposal_v2_2026_07_10.tex` (+`.pdf`) | **CURRENT internal proposal (v2.4, 2026-08-03)**: S1 contract, live two-process DPF/conversion evidence, full-width GPU AES semantics, S2 parameter-reduction, DMPF/direct-ring novelty, provenance, and advisor-decision hard stops; no security or publication-readiness claim; stable filename |
