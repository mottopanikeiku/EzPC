...
## 2026-05-07 Support Audit Note

The submitted abstract was audited against the saved GPU-MPC artifacts in
`results/submitted_abstract_support_audit.md`, generated by
`scripts/audit_submitted_abstract_support.py` from the project root.

Key outcome:

- the DPF, NTT/PolyMul, VOLE validation, and Orca key-read-versus-compute claims are supported by saved artifacts,
- the saved local Orca profile supports key files from hundreds of MiB to about `4.0G` per party, not "tens of gigabytes",
- wording that sounds like completed Orca key replacement should be softened because the current DPF/VOLE results are standalone prototypes and full Orca integration remains ongoing work,
- for poster/camera-ready text, prefer the shortened evidence-safe abstract in `results/submitted_abstract_support_audit.md`.
