...
Updated: 2026-05-16 for q128 Cheddar CRT wiring
...
The VOLE benchmark reuses the promoted cheddar-derived GPU polynomial path rather than introducing a second CUDA polynomial engine. q32/q64 use one prime limb, and q128 uses the same two q62 CRT residue-limb layout as the core Cheddar NTT/PolyMul benchmark.

The current prototype supports:

- requested `q=32`, realized as actual `q=30`,
- requested `q=64`, realized as actual `q=62`,
- requested `q=128`, realized as actual `q=124` with two q62 CRT limbs,
...
- `docker exec -e QBITS=128 -w /home/ringlpn orca-dev bash scripts/run_vole_sweep.sh`
...
- `QBITS=128 bash scripts/run_cuda_sweep.sh`
...
- `results/vole_gpu_q128_smoke.csv`
...
2. The prototype reuses the validated promoted Cheddar GPU polynomial backend rather than a separate ad hoc kernel path.
3. The current prototype validates the intended algebraic VOLE relation coefficient-wise across the full tested degree range from `8192` to `1048576` for both requested `q=32` and requested `q=64`, and the saved q=128 CRT smoke sweep validates the same degree range with `m=2`, `c=2`, and noise weight `8`.
...
4. Claiming full q=128 VOLE sweep numbers before collecting the `QBITS=128` sweep artifact.
...
- On 2026-05-16, the VOLE benchmark was wired through the q128 Cheddar CRT residue-limb layout and passed q32/q64/q128 smoke validation at `n=8192`, `m=2`, `c=2`, and noise weight `8`.
