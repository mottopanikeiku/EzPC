...
number of communication stages is `6L+6` for depth `L` **independent of how many
trees are in the batch**, and 369/369 generated key pairs validate through the
*unchanged* evaluator while reproducing the contract's opening accounting
bit-for-bit.
...
| Wire cost | not measurable | **measured bytes and stages per party** |
...
| `L` | prime | batch | string OTs/tree | triple OTs/tree | OLE OTs/tree | logical bits/tree | revealed bits/tree | P0 batch bytes | P1 batch bytes | stages/batch | us/tree |
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 4 | q62 | 8 | 8 | 6 | 186 | 588 | 1,176 | 45,230 | 46,742 | 30 | 667 |
| 8 | q62 | 8 | 16 | 14 | 186 | 1,116 | 2,232 | 63,254 | 64,766 | 54 | 1,175 |
| 11 | q62 | 4 | 22 | 20 | 186 | 1,512 | 3,024 | 62,974 | 63,718 | 72 | 3,116 |
| 14 | q62 | 2 | 28 | 26 | 186 | 1,908 | 3,816 | 68,671 | 75,175 | 90 | 7,687 |
| 8 | q62b | 8 | 16 | 14 | 186 | 1,116 | 2,232 | 63,254 | 64,766 | 54 | 1,264 |
| 14 | q62b | 2 | 28 | 26 | 186 | 1,908 | 3,816 | 68,671 | 75,175 | 90 | 9,152 |

Measured stage count per batch is exactly `6L+6` (30, 54, 72, 90 at `L`=4, 8,
11, 14). Setup, once per connection per party: **256 base OTs, 21,829 bytes, 6
stages** (two Naor-Pinkas base-OT batches, one per direction).

### 3.2 Batch scaling at `L=11` — the point of batching

| batch trees | stages/batch | P0 bytes/tree | us/tree | per-tree correlations and openings |
|---:|---:|---:|---:|---|
| 1 | 72 | 52,626 | 11,321 | 22 / 20 / 186, 1,512 / 3,024 |
| 16 | 72 | 6,524 | 737 | identical |
| 64 | 72 | 4,349 | 265 | identical |
| 256 | 72 | 3,789 | 161 | identical |

Three readings:

1. **Stages are depth-bound, not batch-bound.** 72 stages at every batch size.
...
3. **Wall clock per tree falls 70x** (11.3 ms -> 161 us) on loopback. This is a
   scheduling result, not a cryptographic one, and it is single-threaded CPU.

Timings are loopback, single-threaded; they are not a throughput claim, and the
stage count is the transport-independent number to quote.
...
- measured stages per batch `= 6L+6` (measured; batching-dependent, not fixed by
  the contract).

**The logical/revealed columns are bit-identical to the frozen contract's closed
...
round/traffic measurement". This artifact discharges the **real transport,
level-synchronous batching, and measured traffic/stages** parts for the keygen
protocol on the host, with real OT rather than silent OT. Still open for M1: a
silent-OT backend, GPU-side batched keygen, byte-identical GPU key emission, and
driving the real-OLE GPU transcript from these keys (M2).
