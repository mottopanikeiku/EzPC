% Dealerless Orca Ring-LPN proposal, v2.6 (2026-08-04).
...
GPU Ring-LPN Design, Proof Boundary, and Evaluation (v2.6)}
\author{Alp}
\date{2026-08-04 \quad (v2.6; canonical correlation IDs and consume-once ledger)}
...
for its linear layers. The corrected source removes that dealer from one
complete forward fully-connected (FC) preprocessing path: two independent
processes sample party-local masks and sparse Ring-LPN noise, fix the unsent
public identity $a_0=1$, jointly sample exact full-field shares only for
$a_1,\ldots,a_{c-1}$, generate standard distributed point-function keys over
SCI/IKNP and Gilboa OLE, expand Figure-2 Ring-LPN OLEs on separate GPUs,
convert exact CRT shares to $\mathbb Z_{2^{32}}$, and emit byte-compatible
Orca keys.
...
At the exact ResNet18 classifier-layer shape $1{\times}512{\times}1000$, ten
current local-loopback trials all pass. Median preprocessing is 25.715\,s and
sends 575{,}846{,}872 application bytes, versus 10.642\,ms for matched stock
trusted-dealer key generation; final payload size remains 4{,}108{,}096 bytes
per party and the unchanged two-share online checker takes 1.106\,ms. This is
...
\item[G3 --- Randomness separation: closed in the live path.] Private roots,
  ...
  Ring-OLE correlation, $a_0=1$ is the unsent public identity polynomial and
  the parties exchange full uniform field-coefficient shares only for
  $a_1,\ldots,a_{c-1}$. Their sums are exact uniform ring elements; a short
  revealed PRG seed is not substituted for this distribution.
...
novel. The systems result is currently a feasibility result: its $2414\times$
median preprocessing/dealer ratio and unpinned Ring-LPN parameters are
publication risks, not competitive-system evidence.
...
\node[int] (coin) {\textbf{D3\ \ exact public-$a$ coin} --- fix unsent $a_0=1$; exchange/add $(c-1)n$ full field-element shares; private state from per-party DRBG};
...
tail slots are discarded.  Both version-2 party records carry $\mathit{iid}$
...
$\mathcal F_{\mathrm{COIN}}^p$ fixes the unsent identity $a_0=1$; each party
sends a canonical uniform $(c-1)n$-coefficient field vector, and componentwise
sums define exact uniform $a_1,\ldots,a_{c-1}$.
...
OpenSSL-private-DRBG roots/noise/masks, fixes the unsent identity $a_0=1$, and
exchanges full uniform field-element shares for the $(c-1)n$ coefficients of
$a_1,\ldots,a_{c-1}$. The exact key-distribution coupling and conversion
simulator close the algebraic hybrid obligations used by the conditional
forward theorem. Concrete Ring-LPN security parameters and an authenticated
deployment remain open; no end-to-end 128-bit claim is made.
...
fixes unsent $a_0=1$ and sets the honest $(c-1)n$-coefficient coin share to the
ideal public tail minus the corrupt share. It retains the corrupt random tape,
...
applies the complete D1 simulator and exact standard-key coupling; replaces
only hidden $X_{1-b}$ and sets
$Z_{1-b}=X_bX_{1-b}-Z_b$; samples each opposing derandomization opening under
that fresh hidden OLE share; applies the exact conversion simulator; and adds
the fresh
uniform output-mask share before serializing
$A_b\Vert B_b\Vert C_b$. Every replacement conditions on prior
state. Across $m$ fresh direction/limb/batch instances, a public-order
hybrid incurs at most the sum of the $m$ role-indexed Figure-2/module-Ring-LPN
advantages; the theorem does not infer a joint claim from one unconditioned
single-instance sample.
This theorem excludes the live
...
sampled and sent by neither party. For each coefficient of
$a_1,\ldots,a_{c-1}$, each party samples and exchanges one full canonical
uniform field-element share; componentwise addition modulo $p$ makes the
$(c-1)n$-coefficient tail exactly uniform if either contribution is uniform.
This costs real traffic, but preserves the required
$a=(1,a_1,\ldots,a_{c-1})$ distribution. Revealing one short PRG seed would
instead restrict the tail to the PRG image and is not substituted without a
...
Measured application traffic includes DPF OT/OLE setup, the full-field
$(c-1)n$ public-tail exchange (with unsent identity $a_0$),
cross-term derandomization openings, conversion correlation and
...
The exact regular-sampler projection audit now replaces the earlier
expected-weight-only approximation with a finite combinatorial law: for each
polynomial, $t$ independently occupied source positions map into the factor's
$d$ buckets; the occupied-bucket count has the corresponding occupancy
distribution, and, conditioned on the projected support, its nonzero
coefficient values are independent uniform elements of $\mathbb F_p^*$.
This proves the implemented sampler's projected-noise distribution, including
collisions and cancellation, but it does not prove the projected
quasi-cyclic code's rank distribution or justify transferring a
random-code decoder cost to that structured code.

Separate source-pinned direct-RSD calculators reproduce published 2024 and
2025 cost tables, and the automorphism orbit of the projected instance is now
derived exactly. Their application to the live tuple remains diagnostic:
the $\sqrt d$ orbit-to-decoder speedup is only heuristic, and the required
structured-code reduction and newer-attack instantiations remain open.
...
G3 shared/private RNG & closed for live artifact & per-party OpenSSL DRBG,
unsent identity $a_0=1$, and $(c-1)n$ exact full-field share exchange &
exact required distribution\\
...
forward artifact. Deployment still requires an executed peer-isolation
boundary, authenticated transport, and a reviewed Ring-LPN parameter pin.}
...
\paragraph{Current live two-process ResNet18-classifier-layer preprocessing
(2026-08-04 identity/freshness rerun).}
The recorded artifact evaluates the exact inference classifier-layer shape
$1{\times}512{\times}1000$ at $\bw=32$ with
$(n,c,t)=(8192,2,8)$, q128, regular noise, $C_{\rm key}=512$, and
$C_{\rm app}=7680$. The source implements GPU-batched distributed DPF
generation with GPU-AES expansion and correction-word reduction; each process
reads only its own private noise record, runs on its own GPU, and emits only
its own final key record. A 128-bit invocation identifier and persistent
consume-once correlation ledger reject replay, duplicate, and stale-tail use.
After both processes exit, the checker reconstructs the records, verifies the
clear product, and invokes unchanged
\texttt{readGPUMatmulKey}/\texttt{gpuMatmulBeaver}.

Ten current measured trials followed one warmup. The corrected public-vector
construction exchanges $(c-1)n$ coefficients per ring batch and treats
$a_0=1$ as public without sending it. The current aggregate CSV and digest-
bound metadata pass all execution, record, transcript, freshness, and
unchanged-online checks; full-model status is
\texttt{FAIL\_UNSUPPORTED} because convolutions and truncation are not part of
this run.
...
mean critical-path preprocessing & $25.699\pm0.058$ s\\
median critical-path preprocessing & $25.715$ s\\
median / matched stock dealer & $2414\times$\\
median matched \texttt{gpuKeygenMatmul} & $10.642$ ms\\
median unchanged two-share online & $1.106$ ms\\
application bytes, both directions & $575{,}846{,}872$\\
transport bytes incl.\ base OT & $575{,}890{,}852$\\
public-$a$ coefficients, both parties & $4{,}128{,}768$\\
protocol dependency rounds & $10{,}338$\\
peak host / GPU bytes & $142{,}493{,}696$ / $27{,}241{,}086{,}976$\\
final payload bytes per party & $4{,}108{,}096$\\
serialized record bytes per party & $4{,}108{,}304$\\
\bottomrule
\end{tabular}
\caption{Exact classifier-layer live result. Time statistics are over ten
current measured trials. Application bytes exclude framing and base OT;
the transport row includes measured base-OT setup.}
\label{tab:modelscale}
\end{table}

The median Phase-C payload-correction stage is 20.432\,s, about 79.4\% of
critical-path preprocessing. Median GPU Ring-LPN expansion is 1.704\,s and
the earlier DPF tree phases are 0.302\,s and 2.588\,s. Thus GPU batching has
removed the old host-only DPF implementation from the live path, but the
per-point payload-correction design remains the dominant systems bottleneck.

The only matched baseline is stock trusted-dealer
\texttt{gpuKeygenMatmul}, measured at the same shape, masks, bit width,
allocator/synchronization boundary, output bytes, and GPU. A separately pinned
stock libOTe Reverse-Cuckoo run completed in 12.43\,s at its nominal
$(2^{20},4,16)$ shape, but it uses Goldilocks arithmetic, internally sampled
factors, a folded 16-group layout, synthetic preloaded base correlations, CPU
local sockets, and no GPU output. It is therefore a reproducible closest
distributed baseline, not a function-matched comparator; no timing ratio is
reported. Current source also contains an opt-in EMP-Silent transport adapter
whose focused full-loopback and representative FC/Conv correctness gates pass,
but no publication timing has been measured for that backend.
...
\begin{description}
\item[Closed --- current live forward path.] Current two-process q64/q128
  smoke rows and the exact classifier run generate distributed GPU-AES DPF
  keys, Ring-LPN OLE shares, converted q32 masks, and byte-compatible Orca FC
  records. Corrupted, swapped, malformed, replayed, duplicated, stale-tail,
  and invocation-collision controls reject; the unchanged online consumer
  reconstructs the expected product.
\item[Closed conditionally --- proof composition.] The theorem composes
  ...
  assumptions with an exact Ring-LPN sample distribution. The exact
  regular-projection law is now executable evidence; the structured-code
  hardness transfer, concrete parameter pin, and independent cryptographic
  review remain open.
\item[Open --- parameters.] A reviewed structured-code and two-CRT-limb
  ...
  feasibility-only.
\item[Open --- performance engineering.] Phase-C payload correction dominates
  current preprocessing. Replace the tree-per-point design with a
  source-reviewed batched DMPF/P-DPF construction, measure the existing
  EMP-Silent backend, reduce dependency rounds, and remeasure stage time,
  traffic, and peak memory. The present $2414\times$ median gap is not a
  competitive result.
\item[Open --- model coverage.] The FC wrapper now supports arbitrary valid
  forward matmul shapes, and a generalized stock-I/F/O Conv2D path passes
  representative q64/q128 smoke cases. Every forward convolution of a real
  model plus its truncation/state handoff must still be executed end to end.
\item[Open --- deployment.] Source contains fail-closed authenticated
  two-host coordination and peer-private container orchestration, but no
  configured second host was available to execute it. Current evidence is
  unauthenticated same-host loopback and cannot predict LAN/WAN latency or
  establish OS-level peer isolation.
\item[Open --- review.] A human cryptographer who did not author the artifact
  must review the DPF/OT/OLE/composition proof and the structured-code
  parameter argument. Internal adversarial review is not a substitute.
...
with a semi-honest proof and prototype. A pinned stock build/run is now
reproduced as the closest distributed implementation baseline, but its field,
factor inputs, folded layout, synthetic setup, CPU transport, and output
contract do not match this artifact, so no ratio is meaningful.
SLAMP-FSS~\cite{kulaots2026slamp} is a second 2026 multi-point design using
tree PRGs and linear systems. The current tree-per-point DPF is GPU-batched,
but it remains a compatibility artifact and baseline rather than a claimed
...
language: a dealer/oracle-free live preprocessing source path now spans
GPU-batched distributed DPF generation, Ring-LPN OLE, slot extraction,
secure q128-to-q32 conversion, byte-compatible key serialization, and
unchanged Orca forward-FC consumption. The exact ResNet18 classifier-layer
run passes one warmup plus ten current measured trials, as do the canonical
q64/q128 controls. Source additionally contains a generalized Conv2D path,
an opt-in EMP-Silent backend, and authenticated peer-isolation launchers; only
their focused local correctness/source checks are complete, not a two-host
deployment or full-model evaluation.

This is a strong internal advisor checkpoint. It is not yet a submission-ready
security/systems paper. Four blockers are decisive: no reviewed concrete-
security parameter point; a roughly $2414\times$ median preprocessing gap to
the matched dealer; no executed authenticated two-host/peer-isolation result;
and novelty currently centered on integration of known
DPF/Ring-LPN/conversion primitives. A cryptography-paper claim would require a
new protocol, reduction, or parameter result; a systems-paper claim would
require competitive performance and model-scale coverage.
...
(2)~replacement of the Phase-C tree-per-point bottleneck by a
source-reviewed DMPF/P-DPF design, followed by EMP-Silent stage, byte,
dependency-round, and memory measurements;
(3)~every forward linear layer and truncation/state handoff of at least one
real inference model, repeated at the pinned parameters;
(4)~authenticated two-host LAN and controlled-WAN execution under distinct
peer-isolation identities; and
(5)~a matched dealer comparison plus separately labeled closest reproducible
dealerless-PCG baselines, followed by renewed source/proof review.
