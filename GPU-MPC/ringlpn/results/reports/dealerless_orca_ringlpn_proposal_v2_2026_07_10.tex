% Dealerless Orca Ring-LPN proposal, v2.14 (2026-08-10).
% Version 2.14 (2026-08-10): retained complete full-graph checkpoint and hardened clean-clone reproduction.
...
GPU Ring-LPN Design, Proof Boundary, and Evaluation (v2.14)}
\author{Alp}
\date{2026-08-10 \quad (v2.14; retained full-graph checkpoint)}
...
public identity $a_0=1$, exchange four uniform 64-bit seed-share words each,
XOR them into one jointly sampled public seed, and use domain-separated
SHAKE256 rejection sampling to derive every exact-uniform Ring-OLE tail in the
explicit random-oracle model. They generate standard distributed point-function
keys with real OT, use external Gilboa OLE only for the first Phase-C epoch of
each CRT limb, reserve earlier Ring-LPN output to bootstrap every later epoch,
expand Figure-2 Ring-LPN OLEs on separate GPUs, convert exact CRT shares to
...
authenticated channels, SHAKE256 modeled as the scoped public-vector random
oracle, and decisional Ring-LPN for the exact sparse distribution, these pieces
give a conditional static-semi-honest theorem for forward FC.

A current 2026-08-10 artifact at the exact ResNet18 classifier-layer shape
$1{\times}512{\times}1000$ records ten passing local-loopback SCI/IKNP trials.
Median post-channel setup-included preprocessing is 4.0193924415\,s and sends
182{,}372{,}344 application bytes, versus 14.73535\,ms for shape-matched stock
trusted-dealer key generation. This timing adds each party's preflight and
selected-backend OT setup to the legacy total, but excludes
\texttt{PartyChannel} construction, socket establishment, and channel
authentication; it is not true end-to-end time. Physical-GPU occupancy was
uncontrolled. Final payload size is 4{,}108{,}096 bytes per party and the
unchanged two-share online checker takes 1.14969\,ms. The current artifact
binary SHA-256 begins \texttt{02eaaac9}.
They remain internal engineering diagnostics rather than publication evidence.
This is a forward-FC preprocessing/checker measurement, not full-model inference or truncation; the
negative comparison establishes feasibility and the next
...
\textbf{New full-graph systems checkpoint (2026-08-10).} A source-bound
q128/bw32 known-zero ResNet18 control now composes all 21 independently
generated convolution/FC records in the exact stock forward order with all 21
secure stochastic truncations, all 19 stock key consumers (one MaxPool, 17
ReLUs, and classifier sign extension), three projection and five identity
residual merges, integer GlobalAvgPool2D, and terminal reconstruction.
Every linear operation uses the unchanged Orca consumer. A test-only trusted
compatibility adapter reads both parties' source-bound linear mask states and
emits the exact stock nonlinear key stream; each live party reads only its own
private record.
Thus this closes the full graph/state-composition seam, not dealerless
nonlinear preprocessing, private or trained inference, accuracy, full-model
performance, deployment, or a new security claim.
\end{abstract}
...
\item[G2 --- Exact share conversion: closed.]
  ...
  implement
  exact $\Z_Q\to\Z_{2^\bw}$ conversion. The canonical and classifier suites use
  SCI/IKNP; EMP-Silent remains an opt-in, independently unreviewed source path.
  The wrap bit is never opened; the simulator in
  \S\ref{sec:design-convert} proves privacy at
...
(1)~A live, trusted-dealer-free two-process forward-FC preprocessing artifact
in the stated random-oracle model,
...
executable local compatibility repair that bootstraps every post-base Phase-C
product from Ring-OLE output;
...
shape/contract-matched stock trusted-dealer comparator whose physical-GPU
occupancy was not controlled;
...
(8)~a digest-pinned clean-clone/container reproducer that rebuilds the paper
and canonical gates while rejecting source mutation and surviving private
scratch.
...
output layer; the professor/contributor credit, permission, disclosure, and
private-project-overlap ruling remains explicitly open and must be settled
before external circulation. The point-DPF and Ring-LPN generator primitives
are prior work, not claimed as novel. The systems result is currently a
feasibility result: the current performance artifact's descriptive
$268.6769431352700\times$ median of setup-included shape-matched per-trial
preprocessing/dealer ratios (without controlled physical-GPU occupancy),
unauthenticated local transport, and unpinned Ring-LPN parameters are
...
SCI/IKNP; EMP-Silent remains opt-in and independently unreviewed. Current
measurements communicate over unauthenticated loopback, so the runs are
...
the separate EMP-Silent source path remains opt-in and independently unreviewed.
The proof models SHAKE256 as a public random oracle and remains conditional on
the unpinned Ring-LPN assumption; the TCP channel lacks authentication.
Therefore neither artifact establishes an end-to-end 128-bit-security claim.
...
base-OT setup bytes. The current runner uses unauthenticated local loopback.
...
q64/q128 and classifier suites use SCI/IKNP. The separate EMP-Silent path is
opt-in and independently unreviewed. Both backends use
\texttt{PartyChannel}/SCI \texttt{NetIO}; neither transport is authenticated.
Application-byte counters exclude backend setup and TCP/IP overhead.
...
artifact. Its \texttt{local-smoke} mode rebuilds this PDF and the full
canonical gate but ends with an explicit ``not two-host publication evidence''
marker. Publication mode additionally requires an owner-authorized annotated
source tag, an externally authenticated tag/commit binding, an immutable
runtime image and in-image binary identity, pinned SSH host identity, a
separate owner-only persistent read-write ledger mount, and the authenticated
two-host/checker manifests. Those release and deployment identities have not
...
\paragraph{Current classifier-layer result (2026-08-10).}
...
unauthenticated single-host timing/traffic rows characterize the current
source. The public-vector construction exchanges four 64-bit seed-share words
per party and layer, then derives a fresh domain-separated SHAKE256 tail per
Ring-OLE while
...
\bottomrule
\end{tabular}
\caption{Exact current classifier-layer result. Time statistics are over ten
...
In the current rows, legacy post-OT median Phase C is 0.041723\,s, Phase B is
...
ResNet18 convolution/FC set through the unchanged Orca linear consumers. The
compiled graph contract is generated from, and hash-checked against, the same
source manifest used by the record-set gate. Its explicit value-source
registry binds every main and shortcut operand; the exact stock forward order
contains 21 linear calls, 21 stochastic truncations, 19 stock key consumers
(one MaxPool, 17 ReLUs, and classifier sign extension), 20 remask edges, eight
residual merges, integer GlobalAvgPool2D, and terminal reconstruction.
...
linear mask states and emits the exact 62-item stock key stream. It took
9.356\,s and produced 1{,}084{,}582{,}352 raw stock-key bytes per party. Each
live party reads only its own private linear, nonlinear, and persistent-state
records. A separate post-exit checker independently reconstructs every linear
input/output, remask, residual, pool, sign-extension, and terminal value from
both published records and rejects counter/order drift. Seven fail-closed
...
The retained 17-file bundle at
\path{results/graph/resnet18_full_graph_checkpoint_2026_08_10/} contains only
metrics, logs, digests, binary approval, and manifests. Private linear,
mask-state, nonlinear, ledger, and output records were deleted. The
orchestration deliberately applies each omniscient per-record correctness
checker before feeding the same test record to this graph control. It therefore
does not evidence consume-once online deployment; production consumption must
omit that test inspection and preserve the deployment-wide one-use boundary.
...
is not private or trained inference, an accuracy result, deployment evidence,
...
allocator/synchronization boundary, and output bytes. The retained executions
were not controlled on the same physical GPU and occupancy state, so the
$268.6769431352700\times$ setup-included statistic is descriptive rather than a hardware-matched A/B result.
A separately pinned stock libOTe Reverse-Cuckoo run completed in 12.43\,s at
its nominal $(2^{20},4,16)$ shape, but it uses Goldilocks arithmetic,
internally sampled factors, a folded 16-group layout, synthetic preloaded base
correlations, CPU local sockets, and no GPU output. It is therefore a
reproducible closest distributed baseline, not a function-matched comparator;
no timing ratio is reported.
The EMP-Silent source path remains opt-in and independently
unreviewed; no current headline measurement uses it.
...
\item[Closed --- current live forward path.] The current focused two-process
  ...
  calls; all 21 shape plans pass. The current FC binary begins
  \texttt{29c420c3} and the focused approval digest begins
  \texttt{843f837a}. These are correctness/path-counter results, not current
  Conv0/model-scale timing or a breadth-first speedup claim. Eleven negative
  controls cover corrupted or swapped records, malformed messages, replay,
  stale state, nonpositive capacity, and publication failure; the unchanged
  online consumer reconstructs the expected product.
\item[Closed narrowly --- full-graph systems control.] One source-bound
  ...
  and output-corruption controls reject. The nonlinear keys come from an
  explicitly trusted test-only adapter that reads both mask states, so
...
\item[Open --- performance engineering.] Phase B remains the largest measured
  ...
  reduction. The current descriptive $268.6769431352700\times$ median of
  setup-included shape-matched per-trial preprocessing/dealer ratios lacks
  same-physical-GPU/occupancy control and is not competitive.
...
forward-FC keys without a trusted dealer in the stated random-oracle model;
those keys match
the stock Orca format and pass its unchanged online computation. A separate
source-bound known-zero control composes a fresh 21-record linear set with the
exact stock ResNet18 graph, using explicitly trusted test-only nonlinear keys.
Conditional semi-honest security applies only to the forward-linear theorem
under its explicit DPF, OT/OLE, authenticated-channel, and exact-distribution
Ring-LPN assumptions.''

The only in-artifact quantitative dealer comparator is stock
\texttt{gpuKeygenMatmul}, aligned on classifier shape, bit width, masks, output
bytes, allocator, and synchronization boundary. The retained runs were not
controlled on the same physical GPU and occupancy state, so they are not a
hardware-matched A/B result. The older single-process Ring-OLE stage timers
omit most live work and are not a baseline.
...
byte-compatible key serialization, and unchanged Orca forward-FC/Conv2D
consumption. The current five-case focused q64/q128 suite integrates the
breadth-first path with positive per-party breadth calls, zero root-to-leaf
calls, and all 21 shape plans passing. The retained older-binary ResNet18
classifier-layer artifact separately passes one warmup plus ten measured trials. A
source-bound known-zero control now composes all 21 fresh linear records with
all 21 secure truncations, the exact 62-item stock stream, every residual
merge, integer GlobalAvgPool2D, sign extension, and terminal reconstruction.
Its nonlinear keys come from a test-only trusted adapter that reads both
parties' mask states; consequently it proves complete graph/state
compatibility, not dealerless nonlinear preprocessing or a wider security
theorem. No private or trained inference, accuracy, timing distribution, or
full-model performance result is claimed. Authenticated peer-isolation
launchers exist, but no two-host execution has occurred. The opt-in EMP-Silent
revision remains independently unreviewed and is not used by headline rows.

This is a strong internal advisor checkpoint. It is not yet a submission-ready
security/systems paper. The known-zero graph/state-composition seam is closed,
but the remaining systems gates are dealerless nonlinear setup, repeated
private-input/trained-model evidence, an authenticated two-host run, and
independent review. Separately, no reviewed concrete-security parameter point
exists, and the current descriptive median of setup-included shape-matched
per-trial preprocessing/dealer ratios is $268.6769431352700\times$ without
controlled physical-GPU occupancy. Novelty is currently
centered on integration/engineering of
...
also requires competitive performance.
