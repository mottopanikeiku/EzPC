% Dealerless Orca Ring-LPN proposal, v2.8 (2026-08-06).
...
GPU Ring-LPN Design, Proof Boundary, and Evaluation (v2.8)}
\author{Alp}
\date{2026-08-06 \quad (v2.8; full-linear systems route checkpoint)}
...
$a_1,\ldots,a_{c-1}$, generate standard distributed point-function keys with
real OT, use external Gilboa OLE only for the first Phase-C epoch of each CRT
limb, reserve earlier Ring-LPN output to bootstrap every later epoch, expand
Figure-2 Ring-LPN OLEs on separate GPUs, convert exact CRT shares to
$\mathbb Z_{2^{32}}$, and emit byte-compatible Orca keys.
...
batches, the three-product payload step, and the noncircular reserved-slot
bootstrap without revealing the control sign. A separate simulator proves the
...
current local-loopback trials all pass. Median preprocessing is 8.942\,s and
sends 159{,}469{,}294 application bytes, versus 10.706\,ms for matched stock
trusted-dealer key generation; final payload size remains 4{,}108{,}096 bytes
per party and the unchanged two-share online checker takes 1.097\,ms. The
measured opt-in EMP SilentFerret revision is independently unreviewed, so these
are internal engineering diagnostics rather than publication evidence. This is
a forward-FC preprocessing/checker measurement, not full-model inference or
truncation; the negative comparison establishes feasibility and the next
optimization target, not a speedup. The exercised $(n,c,t)=(8192,2,8)$ setting
is a correctness point: the primary-source audit found no reviewed
concrete-security parameter pin. The paper therefore makes no 128-bit-security,
WAN, malicious-security, training-layer, or full-dealerless-Orca claim.
\end{abstract}
...
\item[G2 --- Exact share conversion: closed.]
  ...
  implement
  exact $\Z_Q\to\Z_{2^\bw}$ conversion. The canonical suite uses SCI/IKNP; the
  classifier benchmark uses the opt-in EMP-Silent backend. The wrap bit is
  never opened; the simulator in \S\ref{sec:design-convert} proves privacy at
  the correlation boundary. The current TCP transport is unauthenticated.
...
standard generation, complete role-specific batch simulators, and an
executable noncircular Ring-OLE-output bootstrap for every post-base Phase-C
product;
(3)~a tree-block GPU reduction that removes the measured Phase-C atomic
bottleneck;
...
(6)~a reproducible exact-ResNet18-classifier evaluation with ten trials and a
matched stock trusted-dealer baseline.
The point-DPF and Ring-LPN generator primitives are prior work, not claimed as
novel. The systems result is currently a feasibility result: its $831\times$
median preprocessing/dealer ratio, unreviewed measured OT backend, and unpinned
Ring-LPN parameters are publication risks, not competitive-system evidence.
...
regime). The live distributed key generation
...
(\S\ref{sec:design-params}). The canonical suite instantiates SCI/IKNP; the
classifier benchmark selects the opt-in, independently unreviewed EMP-Silent
revision. Both communicate over unauthenticated loopback, so the runs are
...
epoch-zero Gilboa OLE, consume-once Ring-OLE-output bootstrap, exact public-
polynomial sampling, and separate process/GPU state. The canonical suite uses
SCI/IKNP; the classifier run uses an independently unreviewed EMP-Silent
revision. The proof is conditional on the unpinned Ring-LPN assumption, and the
TCP channel lacks authentication; therefore neither artifact establishes an
end-to-end 128-bit-security claim.
...
\[
g_0=Z_0+\Delta_0X_1+\Delta_0\Delta_1,\qquad
g_1=Z_1+\Delta_1X_0.
\]
...
pad independent of the honest input; the sent difference is determined by the
corrupt party's permitted local input/share. This realizes the same ideal OLE
and composes under distinct consume-once slot identifiers. The abstract counts
...
theorem, semi-honest security of the selected OT backend and Gilboa field-
multiplication wrapper, authenticated channels, and the BCG+20 Figure-2
reduction.
...
$X_{1-b}$ and sets $Z_{1-b}=X_bX_{1-b}-Z_b$; then treats that ideal output's
reserved slots as OLE correlations for the next DPF via the masked-difference
lemma above. Induction in public epoch order covers every later DPF and
...
\label{eq:bootstrap}
\end{equation}
The live protocol realizes these epochs. It reserves the prefix
$C_{\rm key}=3c^2t^2$ of every Ring-OLE output, exposes the suffix
$C_{\rm app}=n-C_{\rm key}$ to FC, and rejects $C_{\rm key}\ge n$ before OT
...
reserved slots using the masked-difference OLE realization above. The final
...
the role-indexed ideal Ring-OLE. Its reserved slots are therefore ideal OLE
correlations for the next DPF; repeat in public order. Distinct slot IDs
compose sequentially. The conclusion remains conditional on the multi-instance
Ring-LPN obligation; bootstrapping does not solve that open assumption.
...
q64/q128 suite uses SCI/IKNP; the classifier benchmark selects the opt-in
EMP-Silent backend at its pinned revision. Both use \texttt{PartyChannel}/SCI
\texttt{NetIO}; neither transport is authenticated. Application-byte counters
exclude backend setup and TCP/IP overhead.
...
\paragraph{Current live classifier-layer result (2026-08-06).}
...
Ten current measured trials followed one warmup using the pinned, opt-in
EMP-Silent backend. The backend revision remains independently unreviewed, so
the timing/traffic rows are internal engineering diagnostics. The corrected
public-vector construction exchanges $(c-1)n$ coefficients per Ring-OLE
instance and treats $a_0=1$ as public without sending it. The current v4
aggregate CSV and digest-bound metadata pass all execution, record, transcript,
freshness, bootstrap-accounting, and unchanged-online checks; full-model status
is \texttt{FAIL\_UNSUPPORTED} because convolutions and truncation are not part
of this run.
...
mean critical-path preprocessing & $8.938\pm0.208$ s\\
median critical-path preprocessing & $8.942$ s\\
median / matched stock dealer & $831\times$\\
median matched \texttt{gpuKeygenMatmul} & $10.706$ ms\\
median unchanged two-share online & $1.097$ ms\\
application bytes, both directions & $159{,}469{,}294$\\
total recorded transport & $160{,}618{,}542$\\
public-$a$ coefficients, both parties & $4{,}521{,}984$\\
...
semantic protocol dependency layers & $11{,}298$\\
peak host / GPU bytes & $179{,}636{,}224$ / $27{,}241{,}086{,}976$\\
...
current measured trials. Application bytes exclude OT setup; total transport
includes the selected backend's setup, whose base-OT subcost is not exposed.}
\label{tab:modelscale}
\end{table}

The median Phase-C stage is now 0.182\,s (2.0\% of critical-path
preprocessing), versus 20.432\,s in the preceding artifact. Phase B is now the
largest measured stage at 3.366\,s (37.6\%); Phase A and GPU Ring-LPN expansion
are 1.910\,s and 1.941\,s. The prior/current comparison changes the
bootstrapping algorithm, GPU reduction, and selected OT backend together, so it
is not an isolated microbenchmark attribution. It does establish that the
former Phase-C leaf-atomic bottleneck is absent from the current trace.
...
reported. The measured EMP-Silent revision passes focused full-loopback and
representative FC/Conv gates but remains opt-in and independently unreviewed.
...
\begin{description}
\item[Closed --- current live forward path.] The current two-process q64/q128
  SCI smoke rows and exact EMP classifier run generate distributed GPU-AES DPF
  keys, self-bootstrapped Ring-LPN OLE shares, converted q32 masks, and byte-
  compatible Orca FC records. Eleven negative controls cover corrupted or
  swapped records, malformed messages, replay, stale state, nonpositive
  capacity, and publication failure; the unchanged online consumer
...
\item[Open --- parameters.] A reviewed structured-code and two-CRT-limb
  ...
  feasibility-only.
\item[Open --- performance engineering.] Phase B is now the largest measured
  stage (37.6\%), followed by GPU Ring-LPN expansion and Phase A; Phase C is
  2.0\%. A source- and cost-audited specialized regular-DMPF design attempt
  returned NO-GO: no reviewed candidate preserved the fixed transcript, plain
  Ring-LPN boundary, application-slot budget, stock-key ABI, and deployed-shape
  cost simultaneously. Keep the measured DPF path for the approved full-linear
  systems integration, independently review EMP-Silent, reduce dependency
  layers, and remeasure traffic/memory. The present $831\times$ median dealer
  gap is not competitive.
...
GPU-batched distributed DPF generation, executable Ring-LPN-output bootstrap,
tree-block GPU reduction, slot extraction, secure q128-to-q32 conversion,
byte-compatible key serialization, and unchanged Orca forward-FC consumption.
The exact ResNet18 classifier-layer run passes one warmup plus ten current
measured trials, as do the canonical q64/q128 controls. Source additionally
contains a generalized Conv2D path and authenticated peer-isolation launchers;
only focused local checks cover Conv2D, and no two-host or full-model execution
has occurred. The measured opt-in EMP-Silent revision remains independently
unreviewed.
...
security parameter point; a roughly $831\times$ median preprocessing gap to
the matched dealer with an unreviewed measured OT backend; no executed
authenticated two-host/peer-isolation result; and novelty currently centered
on integration/engineering of known DPF/Ring-LPN/conversion primitives. A
cryptography-paper claim would require a new protocol, reduction, or parameter
result; a systems-paper claim would require competitive performance and
...
(2)~the approved full-forward-linear systems route: all 20 ResNet18
convolutions, its classifier, and every configured truncation/state handoff
through unchanged stock consumers, with exact order/branch/freshness controls;
(3)~independent EMP-Silent review plus renewed stage, byte, dependency-layer,
memory, source, and proof measurements;
...
dealerless-PCG baselines. The specialized regular-DMPF route remains a design
