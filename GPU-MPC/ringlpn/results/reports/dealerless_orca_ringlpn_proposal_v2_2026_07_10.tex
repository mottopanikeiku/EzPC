% Dealerless Orca Ring-LPN proposal, v2.15 (2026-08-14): internal/advisor checkpoint
% Version 2.15 (2026-08-14): retained complete full-graph evidence and hardened clean-clone gate
...
consumers. Five focused q64/q128 cases and sixteen rejection controls pass.
The current ResNet18 classifier row records a 4.0193924415\,s median
setup-included preprocessing time against a 14.73535\,ms shape/contract-matched
stock-dealer median; uncontrolled physical-GPU occupancy makes the descriptive
per-trial ratio median, $268.6769431352700\times$, a negative comparison rather
...
All numerical claims are regenerated from the retained manifests/CSVs named in
\S\ref{sec:baseline}; source, binary, schema, and evidence digests are checked by
...
\item[G2 --- Exact share conversion: closed.]
  ...
  suites use SCI/IKNP; EMP-Silent remains an opt-in, independently unreviewed
  source path. The wrap bit is never opened; the simulator in
  \S\ref{sec:design-convert} proves privacy at the correlation boundary. The
  measured runner first performs mutual HMAC-SHA256 endpoint/context
  establishment; later protocol bytes have no per-message integrity.
...
(6)~an exact-ResNet18-classifier feasibility evaluation with ten trials and a
...
descriptive $268.6769431352700\times$ preprocessing/dealer ratio median,
without controlled physical-GPU occupancy, unprotected post-handshake local
transport, and unpinned Ring-LPN parameters are publication risks rather than
...
measurements use same-host loopback after a mutual HMAC-SHA256
endpoint/context handshake, but later protocol bytes have no per-message
integrity. They are implementation/correctness evidence for the conditional
theorem rather than an authenticated-channel deployment.
...
\item[Closed --- current live forward path.] The current focused two-process
  ...
  Conv0/model-scale timing or a breadth-first speedup claim. Sixteen negative
...
\item[Open --- deployment.] Source contains fail-closed two-host coordination
  and peer-private container orchestration over pinned SSH, but no configured
  second host was available to execute it. Current evidence uses same-host
  loopback with authenticated endpoint/context establishment but no
  post-handshake per-message integrity; it cannot predict LAN/WAN latency,
  establish authenticated protocol-channel integrity, or prove OS-level peer
  isolation.
...
one warmup plus ten measured trials. A source-bound known-zero control composes
...
preflight authenticates local endpoints/context but not later protocol bytes.
Pinned-SSH peer-isolation launchers exist, but no two-host execution has
occurred. EMP-Silent remains opt-in, independently unreviewed, and absent from
headline rows.

This is an internal/advisor checkpoint, not a submission-ready security or
systems paper. The known-zero graph/state-composition seam is closed, but the
remaining systems gates are dealerless nonlinear setup, repeated
private-input/trained-model evidence, authenticated distinct-host execution,
competitive/matched evaluation, and independent review. No reviewed
concrete-security parameter point exists, and the current descriptive median
of setup-included shape-matched per-trial preprocessing/dealer ratios is
$268.6769431352700\times$ without controlled physical-GPU occupancy. Novelty is
centered on integration/engineering of known DPF/Ring-LPN/conversion
