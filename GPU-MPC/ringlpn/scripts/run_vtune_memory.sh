...
if [[ -z "$RESULT_DIR" ]]; then
  RESULT_DIR="$BASE_DIR/results/profiling/vtune_memory_${N}_${QBITS}"
fi
...
vtune -collect memory-access -result-dir "$RESULT_DIR" "$BIN" --n "$N" --qbits "$QBITS" --iters "$ITERS" --warmup "$WARMUP"
vtune -report summary -result-dir "$RESULT_DIR"
vtune -report memory-access -result-dir "$RESULT_DIR"
