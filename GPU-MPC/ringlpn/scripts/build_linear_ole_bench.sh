...
NVCC="${NVCC:-nvcc}"
DEVICE_LABEL="${DEVICE_LABEL:-cuda_ringlpn_linear_ole}"
...
"$NVCC" "${COMMON_FLAGS[@]}" \
  ...
  "$BASE_DIR/src/bench_linear_ole_ringlpn_cuda.cu" \
  "$PROJECT_ROOT/utils/gpu_mem.cu" \
  -o "$OUT_DIR/bench_linear_ole_ringlpn_cuda"

echo "Built:"
ls -la "$OUT_DIR/bench_linear_ole_ringlpn_cuda"
