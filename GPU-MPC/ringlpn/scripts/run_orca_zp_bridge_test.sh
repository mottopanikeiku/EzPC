...
"$BIN" --bw 32 --rows 1 --inner 1 --cols 1 \
...
with md_path.open("w") as f:
    ...
    f.write("- Constant-polynomial scalar packing is valid only under the explicit no-prime-wrap bound `inner * value_bound^2 < p`.\n")
    f.write("- The `bw=32` full-range row is intentionally not claimed; it records a counterexample showing why q62 is insufficient for unrestricted 32-bit scalar products.\n")
PY
