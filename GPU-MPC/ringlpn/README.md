...
## CUDA q=32 sweep
The current primary GPU deliverable is a batched CUDA NTT path for requested `q=32`, realized with one 30-bit prime that supports `n` through `2^20`.
...
- The current roadmap is: primary cheddar-derived q=32 path, then 64-bit Montgomery kernels using `__umul64hi()`, then dual-prime CRT support for requested `q=128`.
