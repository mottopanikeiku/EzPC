...
static void build_inputs(OleState &state) {
    ...
    // Dealerless-keygen hooks (both are off unless the environment sets them and
    // neither changes the benchmark's own behaviour):
    //   RINGLPN_OLE_EXPORT_NOISE=<prefix>  write each party's private noise so the
    //     two-process SPFSS keygen can take its own inputs from disk;
    //   RINGLPN_OLE_SPFSS_KEYS=<prefix>    load SPFSS keys produced by that
    //     two-party protocol instead of calling the centralized GPU keygen.
    if (const char *prefix = std::getenv("RINGLPN_OLE_EXPORT_NOISE")) {
        for (int party = 0; party < 2; ++party) {
            ...
            const std::string path = std::string(prefix) + "_limb" +
                                     std::to_string(state.limb_index) + "_p" +
                                     std::to_string(party) + ".noise";
...
static double load_two_party_spfss_keys(OleState &state, const char *prefix) {
    ...
    ringlpn_spfss_zp::GPUDPFZpKey *dst[2] = {nullptr, nullptr};
    state.keys0.assign(expected_groups, ringlpn_spfss_zp::GPUDPFZpKey{});
    state.keys1.assign(expected_groups, ringlpn_spfss_zp::GPUDPFZpKey{});
    state.spfss_pair_key_bytes = 0;
    for (int party = 0; party < 2; ++party) {
        const std::string path = std::string(prefix) + "_limb" +
                                 std::to_string(state.limb_index) + "_p" +
                                 std::to_string(party) + ".spfss";
        int file_party = -1, file_levels = 0;
        uint64_t file_modulus = 0;
        std::vector<std::vector<spfss_host::DPFKey>> grouped;
        if (!ringlpn_keyio::spfss_groups::read(path, file_party, file_levels,
                                               file_modulus, grouped)) {
        ...
        if (file_party != party || file_levels != state.log_domain ||
            ...
            std::cerr << "two-party SPFSS key file " << path
                      << " does not match this limb (party " << file_party
                      << ", levels " << file_levels << ", groups "
                      << grouped.size() << ")\n";
            std::exit(1);
        }
        dst[party] = (party == 0) ? state.keys0.data() : state.keys1.data();
        for (size_t g = 0; g < expected_groups; ++g) {
            const std::vector<spfss_host::DPFKey> &group = grouped[g];
            ringlpn_spfss_zp::GPUDPFZpKey &key = dst[party][g];
            key.party = party;
            key.log_domain = state.log_domain;
            key.count = static_cast<int>(group.size());
            key.modulus = state.modulus;
            const size_t levels = static_cast<size_t>(state.log_domain);
            key.seeds.resize(group.size());
            key.s_cw.resize(group.size() * levels);
            key.t_l_cw.resize(group.size() * levels);
            key.t_r_cw.resize(group.size() * levels);
            key.final_cw.resize(group.size());
            for (size_t k = 0; k < group.size(); ++k) {
                key.seeds[k] = static_cast<AESBlock>(group[k].seed);
                key.final_cw[k] = group[k].finalCW;
                for (size_t l = 0; l < levels; ++l) {
                    key.s_cw[k * levels + l] =
                        static_cast<AESBlock>(group[k].sCW[l]);
                    key.t_l_cw[k * levels + l] = group[k].tLCW[l];
                    key.t_r_cw[k * levels + l] = group[k].tRCW[l];
                }
            }
            state.spfss_pair_key_bytes +=
                ringlpn_spfss_zp::serializedSizeGPUDPFZpKey(key);
        }
    }
    for (size_t g = 0; g < expected_groups; ++g) {
